...
export function AddressesPage() {
  ...
  const [challenge, setChallenge] = useState<{ address: string; statement: string }>();
  const statement = challenge && challenge.address === account.address ? challenge.statement : '';

  const prepareStatement = () => {
    if (!account.address) return;
    const issued = new Date();
    const issuedAt = issued.toISOString();
    const expiresAt = new Date(issued.getTime() + 10 * 60_000).toISOString();
    setChallenge({
      address: account.address,
      statement: createVerificationStatement({
        address: account.address,
      ...
      {!account.isConnected ? <EmptyState title="Connect a wallet" description="Select a Harmony address before beginning ownership verification." /> : (
        <div className="tic-dashboard-grid">
          <DataPanel>
            ...
            {!statement ? <Button variant="secondary" onClick={prepareStatement}>Prepare verification statement</Button> : <><pre className="tic-mono" style={{ whiteSpace: 'pre-wrap' }}>{statement}</pre><label className="tic-alert"><input type="checkbox" checked={reviewed} onChange={(event) => setReviewed(event.target.checked)} /><span>I reviewed the address, domain, chain, nonce, purpose, and expiration.</span></label></>}
          </DataPanel>
          <DataPanel>
            ...
            <div style={{ height: 'var(--tic-space-lg)' }} />
            <Button disabled={!statement || !reviewed || Boolean(signature)} loading={signMessage.isPending} onClick={requestSignature}><ShieldCheck aria-hidden="true" size={16} /> Request signature</Button>

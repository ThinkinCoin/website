...
use BeycanPress\CryptoPay\PluginHero\Model\AbstractModel;
// Types
...
abstract class AbstractTransaction extends AbstractModel
{
     ...
     * @return bool
     */
    public function addOrderRefundData(string $hash, RefundType $refund, Status $status): bool
    {
        $tx = $this->findOneBy([
            'hash' => $hash
        ]);

        $order = $tx->getOrder()->addRefund($refund);
     ...
     * @return bool
     */
    public function deleteOrderRefundData(string $hash, int $refundId, Status $status): bool
    {
        $tx = $this->findOneBy([
            'hash' => $hash
        ]);

        $order = $tx->getOrder()->deleteRefund($refundId);
    ...
    {
        return $this->updateStatusByHash($hash, Status::FULLY_REFUNDED);
    }

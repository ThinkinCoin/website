...
use BeycanPress\CryptoPay\Services\Verifier;
use BeycanPress\CryptoPay\Services\Converter;
use BeycanPress\CryptoPay\Services\Sanctions;
use BeycanPress\CryptoPay\PluginHero\BaseAPI;
...
class RestAPI extends BaseAPI
{
    ...
    public function middleware(mixed $result, \WP_REST_Server $server, \WP_REST_Request $request): mixed
    {
        ...
        if (strpos($request->get_route(), 'cryptopay') !== false) {
    ...
    public function setReminderEmail(): void
    {
        ...
        Response::success(esc_html("Reminder email set successfully!", 'cryptopay'));
    }
    ...
    {
        if (Helpers::getSetting('cronType') == 'wp') {
    ...
    /**
     * Check sanctions for walleta addresses
     * @return void
     */
    public function sanctions(): void
    {
        Response::success(null, Sanctions::check($this->request->getParam('address')));
    }
    ...
    private function getErrorMessage(string $code): string
    {
        $errorMap = [
            ...
            'PFE102' => esc_html__('Redirect links cannot finded!', 'cryptopay'),
            // @phpcs:ignore

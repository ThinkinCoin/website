	...
	class mycred_tutor_lms_Specific_Lesson_Hook_Class extends myCRED_Hook {
		...
		**/
		public function my_cred_specific_lesson_func($lesson){
			...
			$ref_type  = array( 'ref_type' => 'post', 'lesson' => $lesson );
			...
			if ( !$this->over_hook_limit('tutor_lms_complete_lesson', 'tutor_lms_complete_lesson', $user_id ) ){
		        ...
		        if( $this->prefs['mycred_check_lesson'] == '1' && in_array( $course_id, $this->prefs['tutor_lms_complete_lesson']['select_course'] ) && ( in_array(0, $this->prefs['tutor_lms_complete_lesson']['select_lesson'] ) || in_array( $lesson, $this->prefs['tutor_lms_complete_lesson']['select_lesson'] ) ) ){
					...
					// Make sure this is a unique event
		            if ( $this->core->has_entry( 'tutor_lms_complete_lesson' , NULL , $user_id , $ref_type, $this->mycred_type) ) return;
			 	...
			 	<div class="row">
	                <div class="col-lg-12">
						...
						<div>
							...
							<input type="checkbox" name="<?php echo esc_attr( $this->field_name( 'mycred_check_lesson' ) ); ?>" id="<?php echo esc_attr( $this->field_id( 'mycred_check_lesson' ) ); ?>" value="1" <?php if( $prefs['mycred_check_lesson'] == '1') echo "checked = 'checked'"; ?> />
								        ...
								        foreach ($course_contents as $content => $value){
						                	...
						                	if( isset( $prefs[ 'select_course' ] ) && isset( $prefs[ 'select_lesson' ] ) ) { ?>
							                    ...
							                    <option value="<?php echo esc_attr( $value->ID );?>"<?php
													echo ( $prefs['select_lesson'] != $value->ID && in_array( $value->ID,  $this->prefs['tutor_lms_complete_lesson']['select_lesson'] ) ) ?  'disabled' : '' ?><?php echo selected($prefs[ 'select_lesson' ],$value->ID) ?>>

...
class Assignments {
	...
	public function load_admin_scripts() {
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'tutor-assignments' ) {
			...
			wp_enqueue_style(
				'assignments-css',
				TUTOR_ASSIGNMENTS()->url . 'assets/css/assignments.css',
				'',
				time()
			);
		}
	...
	public function delete_tutor_course_assignment_submission() {
		...
		$submitted_id = isset( $_POST['assignment_id'] ) ? absint( $_POST['assignment_id'] ) : null;
	...
	public function add_assignments_btn( $topic_id ) {
		?>
		<button class="tutor-btn tutor-btn-outline-primary tutor-btn-sm tutor-create-assignments-btn" data-topic-id="<?php echo $topic_id; ?>">
	...
	public function tutor_modal_create_or_update_assignment() {
		...
		$assignment_action = $assignment_id == 0 ? 'create' : 'update';
		...
		$hook_name = $assignment_action == 'create' ? 'tutor_assignment_created' : 'tutor_assignment_updated';
	...
	public function tutor_assignment_submit() {
		tutor_utils()->checking_nonce();
		global $wpdb;
		$update_id            = isset( $_GET['update-assignment'] ) && $_GET['update-assignment'] ? $_GET['update-assignment'] : 0;
		$assignment_id        = (int) sanitize_text_field( tutor_utils()->array_get( 'assignment_id', $_POST ) );
		$assignment_answer    = wp_kses_post( tutor_utils()->array_get( 'tutor-global-text-editor', $_POST ) );
		...
		if ( $allowd_upload_files && is_array( $upload_attachment ) && count( $upload_attachment ) ) {
			if ( $update_id ) {
				// Update attachments.
		...
		wp_safe_redirect( esc_url_raw( tutor_utils()->avalue_dot( '_wp_http_referer', $_POST ) ) );
		exit;
	}
	...
	public function tutor_evaluate_assignment_submission() {
		...
		$assignment_post_id = isset( $_POST['assignment_post_id'] ) ? sanitize_text_field( $_POST['assignment_post_id'] ) : 0;
	...
	public static function remove_assignment_attachment():void {
		...
		$update = update_comment_meta( $assignment_comment_id, 'uploaded_attachments', json_encode( $updated_files ) );

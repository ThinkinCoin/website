...
class Plugin
{
    ...
    public static function attach_hooks()
    {
        ...
        add_action( 'widgets_init', [ ETHPRESS_NS . '\\Widgets\\Button', 'widgets_init' ] );
    ...
    public static function wallet_connect_project_id_admin_notice_warn()
    {
        ...
        if ( empty($options['wallet_connect_project_id']) || $options['wallet_connect_project_id'] === self::WALLET_CONNECT_DEFAULT_PROJECT_ID ) {
            $settings_link = '<a href="' . self::plugin_settings_link() . '" target="_blank">';
            $msg = sprintf(
    ...
    {
        $url = esc_url( add_query_arg( 'page', 'ethpress', get_admin_url() . 'options-general.php' ) );
        if ( is_multisite() && current_user_can( 'manage_network_options' ) ) {
            $url = esc_url( add_query_arg( 'page', 'ethpress', network_admin_url() . 'settings.php' ) );
        }
        return esc_attr( $url );
    }
    ...
    public static function register_scripts()
    {
        ...
        wp_add_inline_script( 'ethpress-login-modal', 'var ethpressLoginWP = ' . wp_json_encode( $inline_script ) . ';', 'before' );
    }
